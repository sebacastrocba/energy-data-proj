...
def run_extract():
    ...
    try:
        from fuel_price.extract import extract_all_data, get_default_data_path, get_project_root
        from fuel_price.config import START_DATE_BRENT, START_DATE_DOLLAR
        
        logger.info("Iniciando extracción de datos...")
        
        # Obtener configuración de Airflow Variables (con fallback a valores por defecto)
        update_all = Variable.get("fuel_etl_update_all", default_var=True, deserialize_json=True)
        brent_start = Variable.get("fuel_etl_brent_start_date", default_var=START_DATE_BRENT)
...
def run_transform():
    ...
    try:
        ...
        # Leer datos raw usando path helper
...
def run_load():
    """Ejecuta el paso de carga del ETL."""
    try:
        ...
        # Leer datos procesados usando path helper
        project_root = get_project_root()
        processed_path = project_root / "data" / "processed"
        
        logger.info(f"Leyendo datos procesados desde: {processed_path}")
        
        # Cargar archivos parquet con nombres CORRECTOS que coinciden con transform.py
        ...
        # CORRECCIÓN: usar "brent_price_monthly" en lugar de "brent_price_aggregated"
        brent_analytics = pd.read_parquet(processed_path / "brent_price_monthly.parquet")
        fuel_analytics = pd.read_parquet(processed_path / "fuel_price_aggregated.parquet")
        usd_ars_analytics = pd.read_parquet(processed_path / "dollar_price_aggregated.parquet")
        ...
        logger.error("Asegúrate de que la tarea de transformación se haya ejecutado correctamente")
        raise
    except Exception as e:
...
with DAG(
    ...
    description='Pipeline ETL de precios de combustibles',
    schedule_interval='0 2 * * *',  # Todos los días a las 2 AM
    start_date=datetime(2024, 1, 1),
    catchup=False,
    tags=['etl', 'fuel_prices'],
) as dag:
