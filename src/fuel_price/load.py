...
def get_postgres_connection(
    ...
    try:
        logger.info(f"Conectando a PostgreSQL: {connection_params['host']}:{connection_params['port']}/{connection_params['database']}")
...
def load_brent_to_staging(
    ...
    try:
        ...
        if truncate:
            ...
            cursor.execute("TRUNCATE TABLE staging.brent_prices RESTART IDENTITY CASCADE;")
        ...
        logger.info(f"Carga completada: {rows_inserted} registros insertados en staging.brent_prices")
...
def load_fuel_to_staging(
    ...
    # Validar columnas requeridas
    required_cols = ['periodo', 'provincia', 'bandera', 'producto', 'precio_surtidor', 'volumen']
    if not all(col in df.columns for col in required_cols):
        raise ValueError(f"DataFrame debe contener columnas: {required_cols}")
    ...
    try:
        if truncate:
            ...
            cursor.execute("TRUNCATE TABLE staging.fuel_prices RESTART IDENTITY CASCADE;")
        ...
        buffer = StringIO()
        df_copy.to_csv(buffer, index=False, header=False, sep='\t', na_rep='\\N', quoting=csv.QUOTE_NONE, escapechar='\\')
        buffer.seek(0)
        ...
        logger.info(f"Carga completada: {rows_inserted} registros insertados en staging.fuel_prices")
...
def load_usd_ars_to_staging(
    ...
    try:
        if truncate:
            ...
            cursor.execute("TRUNCATE TABLE staging.usd_ars_rates RESTART IDENTITY CASCADE;")
        ...
        conn.commit()
        rows_inserted = len(records_list)
        logger.info(f"Carga completada: {rows_inserted} registros insertados en staging.usd_ars_rates")
...
def load_brent_to_analytics(
    ...
    required_cols = ['year', 'month', 'avg_brent_price_usd', 'min_brent_price_usd', 
                     'max_brent_price_usd', 'record_count']
    if not all(col in df.columns for col in required_cols):
        raise ValueError(f"DataFrame debe contener columnas: {required_cols}. "
    ...
    try:
        if truncate:
            ...
            cursor.execute("TRUNCATE TABLE analytics.brent_prices_monthly RESTART IDENTITY CASCADE;")
        ...
        conn.commit()
        rows_inserted = len(records_list)
        logger.info(f"Carga completada: {rows_inserted} registros insertados en analytics.brent_prices_monthly")
...
def load_fuel_to_analytics(
    ...
    required_cols = ['year', 'month', 'provincia', 'bandera', 'producto', 
                     'precio_surtidor_mediana', 'volumen_total']
    if not all(col in df.columns for col in required_cols):
        raise ValueError(f"DataFrame debe contener columnas: {required_cols}. "
                        f"Las agregaciones deben hacerse en transform.py")
    ...
    try:
        if truncate:
            ...
            cursor.execute("TRUNCATE TABLE analytics.fuel_prices_monthly RESTART IDENTITY CASCADE;")
        ...
        conn.commit()
        rows_inserted = len(records_list)
        logger.info(f"Carga completada: {rows_inserted} registros insertados en analytics.fuel_prices_monthly")
...
def load_usd_ars_to_analytics(
    ...
    required_cols = ['year', 'month', 'avg_usd_ars_oficial', 'avg_usd_ars_blue', 
                     'avg_brecha_cambiaria_pct', 'record_count']
    if not all(col in df.columns for col in required_cols):
        raise ValueError(f"DataFrame debe contener columnas: {required_cols}. "
                        f"Las agregaciones deben hacerse en transform.py")
    ...
    try:
        if truncate:
            ...
            cursor.execute("TRUNCATE TABLE analytics.usd_ars_rates_monthly RESTART IDENTITY CASCADE;")
        ...
        conn.commit()
        rows_inserted = len(records_list)
        logger.info(f"Carga completada: {rows_inserted} registros insertados en analytics.usd_ars_rates_monthly")
...
def load_all_data(
    ...
    if not test_connection():
        raise ConnectionError("No se puede conectar a PostgreSQL. Verifica que Docker este corriendo.")
