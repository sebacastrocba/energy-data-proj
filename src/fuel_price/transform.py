...
def clean_brent_price(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"  Rango: {cleaned_df['date'].min().date()} a {cleaned_df['date'].max().date()}")
...
def agg_brent_price(
    ...
    """
    freq_map = {'D': 'diaria', 'W': 'semanal', 'M': 'mensual', 'Q': 'trimestral', 'Y': 'anual'}
...
def clean_fuel_price(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    logger.info(f"Iniciando limpieza de combustibles - Registros iniciales: {len(df):,}")
    ...
    if nulls_removed > 0:
        logger.info(f"  Eliminados {nulls_removed:,} registros con valores nulos en periodo/precio")
    ...
    if productos_sin_mapear > 0:
        logger.warning(f"  {productos_sin_mapear:,} registros con productos no mapeados (serán eliminados)")
    ...
    logger.info(f"Limpieza completada - Registros finales: {len(cleaned_df):,}")
    logger.info(f"  Rango: {cleaned_df['periodo'].min().date()} a {cleaned_df['periodo'].max().date()}")
...
def fuel_price_aggs(
    ...
    df_aggregated = df_selected.groupby(
        ["periodo", "provincia", "bandera", "producto"],
        as_index=False
    ).agg(
...
def fuel_price_aggs_for_analytics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    result_df = df[['year', 'month', 'provincia', 'bandera', 'producto', 
                     'precio_surtidor_mediana', 'volumen_total']].copy()
